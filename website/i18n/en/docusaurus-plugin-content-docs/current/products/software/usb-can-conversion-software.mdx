...
USB-CAN変換ソフトウェアは达妙科技が開発したソフトウェアで、デバッグモジュールを接続することでCANプロトコルパケットを可視化送信できます。
...
USBからCANへの変換でCANプロトコルパケットを送信するには、达妙科技公式のUSB-CAN変換モジュールとの併用が必要です。
...
接続が成功すると、左下の「Disconnect」ボタンが赤と緑で点滅します。
